#!/bin/bash
# Post-render hook: build the Typst PDF deliverables into docs/.
...
# Output filenames are set via `output-file:` in each qmd's typst format so
# navbar links and Quarto's "Other Formats" links stay consistent.
#
...
echo "Rendering PDF deliverables (Typst)..."
# slide.qmd is not a website page (typst-only), so its output lands in the
# project root — move it into the site output dir alongside the others.
